 ...
 <body  onload="button();">
  ...
  <li><a href="form.php" class="reveal" <?php if($_SESSION['Class']== 'Manager') echo  'style="display:none;"';?>><span style="display:inline-block; padding-right:5px;" class="fa fa-wpforms"></span> Leave </a></li>
    ...
    <div class="box-header well" data-original-title="">
        <h2><i class="glyphicon glyphicon-user"></i> Approve Allowance Application</h2>
    ...
    <div class="alert alert-info">Please Click "Approve" Button to approve the applications </div>
   ...
   <th>Status</th>
	...
	$id=$_SESSION['CompanyId'];
	 	$sql = "SELECT * From claimapplication inner join employee on claimapplication.Employee_No = employee.Employee_No Where Status ='Pending' and employee.Company_id = $id";
     $result=mysqli_query($db, $sql);

	while($row=mysqli_fetch_assoc($result)){
		    echo'<tr>
	  <td class ="center">'.$row['CA_ID'].' </td>
        <td class ="center">'.$row['Employee_ID'].' </td>
		 <td class ="center">'.$row['Employee_Name'].' </td>
        <td class="center">'.$row['Claim_Type'].'</td>
        <td class="center">'.$row['Claim_Amounts'].'</td>
		     <td class="center">'.$row['Date'].'</td>
        <td class="center">
            <span class="label-warning label label-default" id="status">'.$row['Status'].'</span>
        </td>
	
		<td class="center">'.$row['Remark'].'</td>
			        <td class="center">
                      <a class="btn btn-info" id="button" data-toggle="modal" onClick="approvei(\'' .$row['CA_ID']. '\',\''.$row['Employee_No'].'\')" href="javascript:void(0)">
                <i class="glyphicon glyphicon-edit icon-white"></i>
                Approve
            </a>
            <a class="btn btn-danger" data-toggle="modal"   onClick="deletei(\'' .$row['CA_ID']. '\',\''.$row['Employee_No'].'\')" href="javascript:void(0)" >
...
function approvei($refnos,$users){
			
			var refId = $refnos;
			var userId = $users;
			SwalApprove(refId,userId);
...
function SwalApprove(refId,userId){
		 
  swal({
    title: 'Approve Application',
    text: 'Are You Really Want to Approve '+userId+ ' Leave Application ?',
    type: 'question',
 ...
 preConfirm: function() {
    ...
    $.ajax ({
      type: "POST",
      url: "updateall.php",
      data: {RfNo: refId, username: userId},
...
function SwalDelete(refId,userId){
  ...
  swal({
    title: 'Are you sure?',
    text: 'You won\'t be able to revert ' +userId+' Application !',
    type: 'warning',
    showCancelButton: true,
    confirmButtonColor: '#3085d6',
    cancelButtonColor: '#d33',
 ...
 preConfirm: function() {
    ...
    $.ajax ({
      type: "POST",
      url: "updateall.php",
      data: {RefNos: refId, userid: userId},
	  success: function(data){
		    swal('Deleted!',''+refId+ 'Your file has been deleted!', 'success'); 
			var tbl = document.getElementById("myTable");
         for (var i=0; i < tbl.rows.length; i++) {
            var trs = tbl.getElementsByTagName("tr")[i];
             var cellVal=trs.cells[0].innerHTML;           				  
